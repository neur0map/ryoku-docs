...
## The desktop music player

The desktop **Music player** widget — one of the wallpaper widgets you can
enable — follows whichever MPRIS player is active (RyoTunes, Spotify, a browser
tab, or `mpv`), with artwork, transport, and a seekbar.

For Spotify, its backdrop can play the track's **Spotify Canvas**. The Canvas
token is only readable inside the Spotify client, so `ryoku doctor` installs
`spicetify-cli` and wires the bundled `ryoku-canvas` extension for you, and
Canvas works out of the box. The setup is inert if you do not run Spotify.
RyoTunes (YouTube Music) stays the default music app; Spotify is a supported
choice.

## Screenshots

Press `Mod + Shift + S` to open **Ryoshot**, the screenshot and annotation tool.
...
| Voice dictation | `Mod + Grave` | Tap to start Voxtype speech-to-text, tap again to stop |
