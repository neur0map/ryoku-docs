...
<CardGroup cols={2}>
  ...
  <Card title="Theming" icon="palette" href="/docs/theming">
    Full-system themes, the matugen palette, and wallpapers.
  </Card>
</CardGroup>
...
The knobs most people reach for first live in **Ryoku Settings** (`Mod+,`), and
the [first-run welcome](/docs/first-boot) offers the same handful inline:

- **Interface scale.** If everything looks oversized on first boot, ease the
  interface scale down and the whole shell resizes. This is the single most
  common first-boot fix.
- **The bar.** Choose **QS Bar**, the full top bar Ryoku ships with, or
  **Sumi**, a minimal left rail. Bar Studio arranges the rails and widgets on
  either one.
- **Desktop widgets.** Toggle the Clock, Calendar, and Music player on or off in
  **Settings > Desktop Widgets**, then drag them where you want on the wallpaper.
- **Colours.** The palette follows your wallpaper by default (derived by
  `matugen`), so a new wallpaper recolors the whole desktop; lock a fixed
  palette instead if you prefer. See [Theming](/docs/theming).

Deeper appearance work — rounding, blur, gaps, animations, the shell palette —
stays in Ryoku Settings too. This page picks up where the GUI stops.
